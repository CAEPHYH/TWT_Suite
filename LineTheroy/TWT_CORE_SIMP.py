...
def detailed_calculation(I, V, Kc, Loss, p_SWS, N_unit, w, t, Fn_K, f0_GHz, Vpc):
    """返回包含中间结果和最终增益的字典"""
    try:
        Lineparam=simple_calculation(I, V, Kc, Loss, p_SWS, N_unit, w, t, Fn_K, f0_GHz, Vpc)
        #         return {
        #     "小信号增益因子C": C,
        #     "互作用长度N": N,
        #     "慢波线上损耗L": L,
...
def simple_calculation(I, V, Kc, Loss, p_SWS, N_unit, w, t, Fill_Rate, f0_GHz, Vpc):
    try:
        # 单位转换
        p_SWS *= 1e-3  # mm转m
        w *= 1e-3
        ...
        R = calculate_Rowe(beta_Space, a=r_beam * Fill_Rate, b=r_beam)  # R=Fn
