...
def solveTWTNOLINE_INIT(C, b, d, wp_w, Rn, m, A0, y_end, N_steps=1000):
    """四阶龙格库塔法求解行波管非线性方程组
    ...
    def compute_derivatives(y, state):
        ...
        F1z_kernel = np.sin(phi_diff) * Rn**2 / (2 * np.pi)
        # F1z_kernel = -F1z_kernel * 0
        F1z_integral = (
            np.sum(F1z_kernel / denominator[np.newaxis, :], axis=0) * delta_phi0
        )
