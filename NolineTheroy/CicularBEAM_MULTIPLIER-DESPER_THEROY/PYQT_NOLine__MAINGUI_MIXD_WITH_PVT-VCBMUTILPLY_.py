...
from Noline_GAIN_MAINCALL_VCBMULTIPLIER_CORE_SUPER_MIX_WITH_PVT import calculate_SEGMENT_TWT_NOLINE
...
MAX_WORKERS = max(1, 32)  # 保留1个核心给系统
...
def calculate_twt_point(params):
    ...
    fixed, freq, kc_before, loss_before, vpc_before, kc_harmonic, loss_harmonic, vpc_harmonic = params
    ...
    try:
        ...
        param_str = (
            ...
            f"  倍频前参数:\n"
            f"    Kc: {kc_before} Ω\n"
            f"    Loss_perunit: {loss_before}\n"
            f"    Vpc: {vpc_before} c\n"
            f"  倍频段参数:\n"
            f"    Kc: {kc_harmonic} Ω\n"
            f"    Loss_perunit: {loss_harmonic}\n"
            f"    Vpc: {vpc_harmonic} c\n"
        )
        ...
        result = calculate_SEGMENT_TWT_NOLINE(
            I=fixed['i'],
            V=fixed['v'],
            Kc=[float(kc_before), float(kc_harmonic)],  # 倍频前和倍频段的Kc
            Loss_perunit=[float(loss_before), float(loss_harmonic)],  # 倍频前和倍频段的损耗
            ...
            f0_GHz=float(freq),
            Vpc=[float(vpc_before), float(vpc_harmonic)],  # 倍频前和倍频段的Vpc
...
class TWTCalculator(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.import_btn = QPushButton("导入CSV模板")
        self.import_btn.clicked.connect(self.import_csv_template)
    ...
    def import_csv_template(self):
        """导入CSV模板数据"""
        path, _ = QFileDialog.getOpenFileName(self, "导入CSV模板", "", "CSV文件 (*.csv)")
        if not path:
            return
        ...
        try:
            with open(path, 'r', encoding='utf-8') as f:
                ...
                # 查找表头索引
                headers = rows[0]
                freq_idx = headers.index("Freq (GHz)")
                kc_before_idx = headers.index("倍频前Kc (Ω)")
                loss_before_idx = headers.index("倍频前Loss_perunit")
                vpc_before_idx = headers.index("倍频前Vpc (c)")
                kc_harmonic_idx = headers.index("倍频段Kc (Ω)")
                loss_harmonic_idx = headers.index("倍频段Loss_perunit")
                vpc_harmonic_idx = headers.index("倍频段Vpc (c)")
                ...
                for row_data in rows[1:]:
                    ...
                    # 添加行
                    self.table_before.insertRow(self.table_before.rowCount())
                    self.table_harmonic.insertRow(self.table_harmonic.rowCount())
                    
                    # 设置倍频前表格数据
                    self.table_before.setItem(
                        self.table_before.rowCount()-1, 0, 
                        QTableWidgetItem(row_data[kc_before_idx] if kc_before_idx < len(row_data) else "")
                    )
                    self.table_before.setItem(
                        self.table_before.rowCount()-1, 1, 
                        QTableWidgetItem(row_data[loss_before_idx] if loss_before_idx < len(row_data) else "")
                    )
                    self.table_before.setItem(
                        self.table_before.rowCount()-1, 2, 
                        QTableWidgetItem(row_data[freq_idx] if freq_idx < len(row_data) else "")
                    )
                    self.table_before.setItem(
                        self.table_before.rowCount()-1, 3, 
                        QTableWidgetItem(row_data[vpc_before_idx] if vpc_before_idx < len(row_data) else "")
                    )
                    
                    # 设置倍频段表格数据
                    self.table_harmonic.setItem(
                        self.table_harmonic.rowCount()-1, 0, 
                        QTableWidgetItem(row_data[kc_harmonic_idx] if kc_harmonic_idx < len(row_data) else "")
                    )
                    self.table_harmonic.setItem(
                        self.table_harmonic.rowCount()-1, 1, 
                        QTableWidgetItem(row_data[loss_harmonic_idx] if loss_harmonic_idx < len(row_data) else "")
                    )
                    self.table_harmonic.setItem(
                        self.table_harmonic.rowCount()-1, 2, 
                        QTableWidgetItem(row_data[freq_idx] if freq_idx < len(row_data) else "")
                    )
                    self.table_harmonic.setItem(
                        self.table_harmonic.rowCount()-1, 3, 
                        QTableWidgetItem(row_data[vpc_harmonic_idx] if vpc_harmonic_idx < len(row_data) else "")
                    )
                
                self.log_message(f"已从 {path} 导入 {self.table_before.rowCount()} 行色散数据")
    ...
    def parse_params(self):
        ...
        # 验证固定参数
        if not fixed_params.get('p_sws') or len(fixed_params['p_sws']) != 2:
            QMessageBox.warning(self, "参数错误", "p_SWS 需要2个值（倍频前和倍频段）")
            return None, None
