...
class TWTCalculator(QMainWindow):
    ...
    def init_ui(self):
        self.setWindowTitle('TWT计算器 v4.3 (支持多段计算)-仅适用于圆形束流')
    ...
    def calculate(self):
        try:
            ...
            with open(save_path, 'w', encoding='utf-8') as f:
                f.write("Frequency(GHz),Combined_power(dB),Kc(Ohm),Loss,Vpc(c),N_Units\n")
                ...
                for p in var_params:
                    ...
                    f.write(f"{p['Freq']},{total_power},{p['Kc']},{p['Loss']},{p['Vpc']},\"{n_units}\"\n")
            ...
            ax.set_ylabel("Combined-power (dB)")
        ...
        if path:
            with open(path, 'w') as f:
                for idx, (data, params, label) in enumerate(self.history_results):
                    ...
                    f.write(f"# Frequency(GHz)\tpower(dB)\n")
                    f.write("# ----------------------------------------\n")
                    for freq, power in sorted(data, key=lambda x: x[0]):
                        f.write(f"{freq:.2f}\t{power:.2f}\n")
                    f.write("\n\n")
