...
from Noline_GAIN_MAINCALL_V2CORE_SUPER import Noline_CORE_CALC_SUPER as Noline_CALC

# 常量定义
CONFIG_DIR = "config"
RESULTS_DIR = "Results"
TABLE_COLUMNS = ["Kc (Ω)", "Loss", "Freq (GHz)", "Vpc (c)"]
PARAM_CONFIG = {
...
class TWTCalculator(QMainWindow):
    ...
    def save_results(self, results, fixed_params):
        ...
        with open(save_path, 'w', encoding='utf-8') as f:
            f.write("Frequency(GHz),Power(W),Kc(Ohm),Loss,Vpc(c)\n")
            for params, (freq, power) in zip(self.get_var_params(), results):
                f.write(f"{freq},{power},{params['Kc']},{params['Loss']},{params['Vpc']}\n")
