...
from Noline_GAIN_MAINCALL_VSHEETBEAMCORE_MIX import calculate_SEGMENT_TWT_NOLINE
...
class ParamWidget(QWidget):
    ...
    def slider_changed(self, value):
        if self.step_type == "linear":
            ...
            actual_value = self.base_value * (1 + 0.01 * value)
...
class TWTCalculator(QMainWindow):
    def __init__(self):
        ...
        self.param_ranges = {
            ...
            'w': (0, None), 't': (0, None),
            'Kc': (0, None), 'Loss': (0, None),
            'Freq': (0.1, None), 'Vpc': (0.1, 0.99),
            'Fn_K': (0, None),
            'P_in': (0, None),  # 新增输入功率参数
            'Loss_attu': (0, None)  # 新增衰减常数参数
        }
        self.step_types = {
            'i': 'linear', 'v': 'linear', 'Fn_K': 'linear',
            'p_sws': 'percent', 'n_unit': None,  # 禁用n_unit滑块
            'w': 'percent', 't': 'percent',
            'P_in': 'percent', 'Loss_attu': 'percent'  # 新增参数步进类型
        }
        self.plot_counter = 1
    ...
    def init_ui(self):
        self.setWindowTitle('非线性圆形束TWT计算器 v4.3 (PyQt5),带状束流圆形束流通吃 (支持多段计算)')
        ...
        params = [
            ...
            ('P_in', '输入功率 P_in (W)'),  # 新增输入功率
            ('Loss_attu', '衰减常数 Loss_attu (dB/m)')  # 新增衰减常数
        ]
        ...
        # 设置默认值
        self.param_widgets['P_in'].line_edit.setText("0.1")
        self.param_widgets['Loss_attu'].line_edit.setText("20")
        ...
        self.table.setHorizontalHeaderLabels(["Kc (Ω)", "Loss", "Freq (GHz)", "Vpc (c)"])
    ...
    def calculate(self):
        try:
            ...
            with open(save_path, 'w', encoding='utf-8') as f:
                f.write("Frequency(GHz),Combined_power(W),Kc(Ohm),Loss,Vpc(c),N_Units,P_in(W),Loss_attu(dB/m)\n")
                ...
                for p in var_params:
                    ...
                    full_args = [
                        ...
                        p['Freq'],
                        p['Vpc'],
                        fixed_params['P_in'],  # 新增输入功率
                        fixed_params['Loss_attu']  # 新增衰减常数
                    ]
                        
                    try:
                        result = calculate_SEGMENT_TWT_NOLINE(*full_args)
                    ...
                    current_results.append((p['Freq'], total_power))
                    f.write(f"{p['Freq']},{total_power},{p['Kc']},{p['Loss']},{p['Vpc']},\"{n_units}\",{fixed_params['P_in']},{fixed_params['Loss_attu']}\n")
            ...
            ax.set_title(f"Combined-power\n(Time {timestamp.replace('_', ' ')})")
            ax.set_xlabel("Frequncy (GHz)")
            ax.set_ylabel("Combined-power (W)")
        ...
        if path:
            with open(path, 'w') as f:
                for idx, (data, params, label) in enumerate(self.history_results):
                    ...
                    f.write(f"# Frequency(GHz)\tpower(W)\n")
                    f.write("# ----------------------------------------\n")
