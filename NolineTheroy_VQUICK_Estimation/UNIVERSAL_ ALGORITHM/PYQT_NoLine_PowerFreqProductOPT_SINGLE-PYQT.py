...
from Noline_GAIN_MAINCALL_V2CORE_ import Noline_CORE_CALC as Noline_CALC
...
class OptimizationWorker(QThread):
    ...
    def initialize_particles(self):
        ...
        # 50%粒子在边界内均匀分布
        for _ in range(int(self.num_particles * 0.5)):
            particles.append(Particle(self.dim, self.bounds))

        # 30%粒子在中心区域
        for _ in range(int(self.num_particles * 0.3)):
            center_bounds = []
            for min_val, max_val in self.bounds:
                center = (min_val + max_val) / 2
                range_val = (max_val - min_val) * 0.3
                center_bounds.append((center - range_val, center + range_val))
            particles.append(Particle(self.dim, center_bounds))

        # 20%粒子在边界附近
        for _ in range(int(self.num_particles * 0.2)):
            edge_bounds = []
            for min_val, max_val in self.bounds:
                if random.random() < 0.5:
                    edge_bounds.append((min_val, min_val * 1.1))
                else:
                    edge_bounds.append((max_val * 0.9, max_val))
            particles.append(Particle(self.dim, edge_bounds))
    ...
    def evaluate_fitness(self, positions):
        ...
        # 并行计算适应度
        results = self.pool.starmap(calculate_power_fitness, args_list)
        return results


def calculate_power_fitness(fixed_params, var_params):
    """计算适应度（功率曲线面积）"""
    try:
        # 计算每个频率点的功率
        results = []
        ...
        powers = [r[1] for r in sorted_results]
...
class TWTOptimizer(QMainWindow):
    ...
    def create_left_panel(self):
        ...
        self.history_combo.currentIndexChanged.connect(self.update_plot)
    ...
    def create_optimization_tab(self):
        ...
        # 最佳功率面积标签
        self.best_fitness_label = QLabel("最佳功率面积: -")
        opt_layout.addWidget(self.best_fitness_label)

        # 平均功率面积标签
        self.avg_fitness_label = QLabel("平均功率面积: -")
        opt_layout.addWidget(self.avg_fitness_label)

        # 速度信息
        self.parallel_info = QLabel(f"并行进程数: {cpu_count()} 个核心")
        opt_layout.addWidget(self.parallel_info)
    ...
    def calculate(self):
        ...
        try:
            ...
            # 计算当前配置的功率面积
            area = self.calculate_power_area(results)

            QMessageBox.information(
                self,
                "完成",
                f"成功计算{len(var_params)}组数据\n" f"功率曲线面积: {area:.4f} W·GHz",
            )
        except Exception as e:
    ...
    def calculate_power_area(self, results):
        """计算功率曲线面积"""
        if len(results) < 2:
            return 0.0
    ...
    def save_results(self, results, fixed_params):
        ...
        with open(save_path, "w", encoding="utf-8") as f:
            f.write(f"# 功率曲线面积: {area:.4f} W·GHz\n")
        ...
        # 保存到历史记录
        label = f"配置{self.plot_counter} (面积: {area:.2f} W·GHz)"
    ...
    def update_plot(self, results=None, fixed_params=None):
        """更新绘图 - 包含历史曲线叠绘"""
        if results is None or fixed_params is None:
            # 仅更新显示历史曲线
            results = []
            fixed_params = self.get_fixed_params()
        ...
        for idx, (data, params, label) in enumerate(self.history_results):
            ...
            ax.plot(freqs, powers, "o-", markersize=4, label=label, alpha=0.7)

        # 绘制当前选中的历史曲线
        if selected_history >= 0 and selected_history < len(self.history_results):
            data, params, label = self.history_results[selected_history]
            freqs, powers = zip(*sorted(data, key=lambda x: x[0]))
            ax.plot(freqs, powers, "o-", markersize=4, label=label, linewidth=2)

        # 绘制当前计算结果（如果不是历史记录的一部分）
        if results:
    ...
    def start_optimization(self):
        ...
        self.best_fitness_label.setText("最佳功率面积: -")
        self.avg_fitness_label.setText("平均功率面积: -")
    ...
    def update_optimization_progress(self, iteration, best_fitness, avg_fitness):
        ...
        self.best_fitness_label.setText(f"最佳功率面积: {best_fitness:.6f} W·GHz")
        self.avg_fitness_label.setText(f"平均功率面积: {avg_fitness:.6f} W·GHz")
    ...
    def update_iteration_results(self, iteration, best_fitness, best_position):
        ...
        # 每5次迭代更新最佳粒子曲线
        if iteration % 5 == 0:
            self.plot_best_particle_curve(best_position)

    def plot_best_particle_curve(self, position):
        """绘制最佳粒子对应的功率曲线"""
        fixed_params = self.get_fixed_params()
        base_var_params = self.get_var_params()
        ...
        # 创建临时参数集
        var_params = []
        for i, vpc in enumerate(position):
            params = base_var_params[i].copy()
            params["Vpc"] = vpc
            var_params.append(params)
        ...
        if results:
            freqs, powers = zip(*sorted(results, key=lambda x: x[0]))
            ax.plot(freqs, powers, "b-o", markersize=4)
            area = self.calculate_power_area(results)
            ax.set_title(f"当前最佳功率曲线 (面积: {area:.4f} W·GHz)")
    ...
    def finish_optimization(self, best_position, best_fitness, fitness_values):
        ...
        # 计算原始功率面积
        original_area = self.calculate_original_power_area()

        # 显示结果
        message = f"优化完成！最佳功率面积: {best_fitness:.4f} W·GHz\n"
        if original_area > 0:
    ...
    def calculate_original_power_area(self):
        """计算原始配置的功率面积"""
        fixed_params = self.get_fixed_params()
        var_params = self.get_var_params()
        ...
        try:
            ...
            return self.calculate_power_area(results)
        except:
            return 0.0
    ...
    def update_results_plot(self):
        ...
        ax.set_title("功率曲线面积分布")
        ax.set_xlabel("面积 (W·GHz)")
        ax.set_ylabel("粒子数量")
        ax.grid(True, linestyle="--", alpha=0.7)
        ...
        ax.axvline(
            ...
            color="r",
            linestyle="--",
            label=f"最佳值: {self.best_fitness:.4f} W·GHz",
        )
        ax.legend()
    ...
    def save_optimization_results(self):
        ...
        with open(save_path, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(
                ["优化结果", f"最佳功率面积: {self.best_fitness:.6f} W·GHz"]
            )
            writer.writerow([])
    ...
    def export_data(self):
        ...
        with open(path, "w", encoding="utf-8") as f:
            for idx, (data, params, label) in enumerate(self.history_results):
                ...
                # 计算功率面积并输出
                area = self.calculate_power_area(data)
                f.write(f"# 功率曲线面积: {area:.6f} W·GHz\n")
