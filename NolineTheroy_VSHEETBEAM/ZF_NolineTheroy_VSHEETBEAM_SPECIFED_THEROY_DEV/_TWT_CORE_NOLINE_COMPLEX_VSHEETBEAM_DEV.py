...
from _TWT_CORE_NOLINE_CALC_EspDEV import compute_esc_V2Z,compute_Esp_V2,compute_Esp_VR


def solveTWTNOLINE_INIT_V2(C, b, d, beam_params, m, A0, y_end, N_steps=1000):
    """四阶龙格库塔法求解行波管非线性方程组
    ...
    def compute_derivatives(y, state):
        ...
        Ez = compute_Esp_V2(phi_diff, denominator,C, m, beam_params)
        print(Ez)
        term_u = (
...
def solveTWTNOLINE_INIT_V2Z(C, b, d, beam_params, m, A0, y_end, N_steps=1000):
    """四阶龙格库塔法求解行波管非线性方程组
    ...
    def compute_derivatives(y, state):
        ...
        Ez = compute_esc_V2Z(phi_diff, denominator,C, m, beam_params)
        print(Ez)
        term_u = (
            A * (1 - C * dtheta_dy) * np.sin(phi) - C * dA_dy * np.cos(phi) + Ez
        )  # ((wp_w/C)**2)/(1 + C*b) * F1z_integral
...
def solveTWTNOLINE_INITR(C, b, d, beam_params, m, A0, y_end, N_steps=1000):
    """四阶龙格库塔法求解行波管非线性方程组
